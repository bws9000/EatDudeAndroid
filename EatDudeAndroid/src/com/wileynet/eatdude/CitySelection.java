...
public class CitySelection extends Activity implements Runnable {
	...
	protected void onStart() {
		...
		if (info.size() < 1) {
			...
			cities = new SpinnerData[info.size() + 1];
			cities[0] = new SpinnerData("select", "select");

			for (Map.Entry<String, String> c : info.entrySet()) {
				// System.out.println("-----> " + c.getValue() +
				// " -- " +
				// c.getKey() );
				cities[count] = new SpinnerData(c.getValue(),
						c.getKey());
				count++;
			}
		...
		if (adapter != null) {
			...
			spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
				public void onItemSelected(AdapterView<?> parent, View v,
						int pos, long id) {
					SpinnerData d = cities[pos];
					String sval = d.getValue();
					MainHome.restaurant_city = d.getSpinnerText();
					...
					if (sval.equals("select")) {
						...
						Intent intent = new Intent(CitySelection.this,
								RestaurantSelection.class);
						intent.putExtra("city_id", sval);
	...
	public void run() {
		...
		try {
			if (ni.isConnected()) {
				String url = remote_app_address + "/xml/city/" + state_id;
	...
	private Handler errorhandler = new Handler() {
		@Override
		public void handleMessage(Message msg) {
			Toast.makeText(CitySelection.this, errorCopy, Toast.LENGTH_SHORT)
					.show();
		}
	};
	...
	private Handler emptyhandler = new Handler() {
		@Override
		public void handleMessage(Message msg) {
			Toast.makeText(CitySelection.this, "please try again later",
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.restaurant_details_inactive:
			...
			return true;
		case R.id.home:
			Intent intent = new Intent(CitySelection.this, MainHome.class);
			startActivity(intent);
			return true;
		case R.id.help:
			Intent intent2 = new Intent(CitySelection.this,
					HelpHome.class);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			...
			try {
				...
				Toast.makeText(CitySelection.this, errorCopy,
						Toast.LENGTH_SHORT).show();
				return false;
			}
		}
